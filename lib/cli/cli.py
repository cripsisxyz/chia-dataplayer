...
class Cli():
    ...
    def argparse():
        ...
        datastores.add_argument('--codec', '-c', type=str, help='Encoding type for reading or updating (default hex)', choices=['hex', 'cose'], default='hex', required=False)
        datastores.add_argument('--action', '-a', type=str, help='Action to execute over datastore (required)', choices=['update_key', 'read_key', 'list_keys', 'delete_key'], required=True)
        datastores.add_argument('--key', '-k', type=str, help='Key of selection (required for read_key, update_key, delete_key actions)', required=False)
        ...
        if args_datastores.action == "update_key":
            ...
            if args_datastores.codec == "cose":
                if args_datastores.cose_ph is not None:
                    try:
                        cose_ph = dict()
                        for i in args_datastores.cose_ph:
                            kv = i.split("=")
                            cose_ph[kv[0]] = kv[1]
                    except Exception as e:
                        logging.error("Malformed k=v pair for cose public headers")
                        exit(1)
                    else:
                        args_datastores.cose_ph = cose_ph
                else:
                    args_datastores.cose_ph = {}
                        
                if args_datastores.cose_uh is not None:
                    try:
                        cose_uh = dict()
                        for i in args_datastores.cose_uh:
                            kv = i.split("=")
                            cose_uh[kv[0]] = kv[1]
                    except Exception as e:
                        logging.error("Malformed k=v pair for cose public headers")
                        ...
                        args_datastores.cose_uh = cose_uh
                else:
                    args_datastores.cose_uh = {}
